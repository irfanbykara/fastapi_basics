...
def test_get_all_posts(authorized_client,test_posts):
    ...
    # assert posts_list[0].Post == test_posts[0].id
...
def test_get_one_post_not_exist(authorized_client,test_posts):
    res = authorized_client.get("/posts/23423")
...
def test_create_post_default_published_true(authorized_client,test_user,test_posts):
    res = authorized_client.post(
        "/posts/", json={"title": "arbitrary", "content": "asdfsdfdsf",})
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "arbitrary"
    assert created_post.content == "asdfsdfdsf"
...
def test_delete_post_not_exist(authorized_client,test_posts,test_user):
    res = authorized_client.delete(f"/posts/243",
                    )
    assert res.status_code == 404
